...
(describe "org-canvas-sync-assignment-group-at-point duplicate guard (issue #179)"
        ...
        (cl-letf (((symbol-function 'org-canvas--log-error)
                  ...
                  ((symbol-function 'org-canvas-api-request)
                   (lambda (method url &rest _)
                     ...
                     (pcase method
                       ('GET [((id . 77) (title . "Homework") (name . "Homework")
                               (updated_at . "2026-01-01T00:00:00Z"))])
                       ('PUT '((id . 77) (title . "Homework") (name . "Homework")
                               (updated_at . "2026-01-02T00:00:00Z")))
                       (_ '((id . 900) (title . "Homework") (name . "Homework"))))))
                  ((symbol-function 'display-buffer) #'ignore))
          (with-temp-org-buffer "* Groups\n** Homework\n:PROPERTIES:\n:WEIGHT: 20\n:END:\n"
            (goto-char (point-min))
            (search-forward "Homework")
            (org-back-to-heading t)
            (org-canvas-sync-assignment-group-at-point)
            (expect errors :to-be nil)
            (expect (cl-some (lambda (r) (eq (car r) 'POST)) requests) :to-be nil)
            (expect (cl-some (lambda (r) (and (eq (car r) 'PUT)
                                              (string-match-p "assignment_groups/77$" (cadr r))))
                             requests)
                    :to-be-truthy)
            (goto-char (point-min))
            (search-forward "Homework")
