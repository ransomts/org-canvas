...
- *15 Content Types*: Assignments, quizzes, pages, modules, rubrics, outcomes, discussions, announcements, files, assignment groups, group categories, sections, per-section date overrides, calendar events, and course settings
...
This prompts for your Canvas URL, API token, and course ID, tests the connection, writes =org-canvas-credentials.el=, and optionally creates skeleton =.org= files for all 14 content types.
...
- Complete file format specifications for all 15 content types
- All supported properties and link resolution between files
- Quiz question types and answer formats
- Course settings (timezone, homepage, grading, license, navigation tabs, course image)
- Group categories, calendar events, sections, and per-section date overrides
- Inline image upload for page/announcement/discussion bodies
