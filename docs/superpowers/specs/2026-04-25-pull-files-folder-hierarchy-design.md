...
The "fresh vs re-pull" branch is detected by scanning `files.org` for any existing heading with `CANVAS_ID`. If any exist, current flat upsert behavior runs and a one-line warning is logged. If none exist (file missing, empty, or only contains `#+TITLE:` and comments), the new fresh-tree emitter runs.
...
### `org-canvas--file-pull-fresh-mode-p`
Predicate. Scans the `files.org` buffer for any entry with a `CANVAS_ID` property. Returns `t` if none found (fresh-pull path) or `nil` (re-pull path).
...
1. Clear caches; fetch folders → folder map.
2. Fetch files (paginated).
3. Open `files.org`; if `fresh-mode-p`, call `emit-fresh-tree`. Otherwise loop over files using current upsert behavior, log a one-line warning suggesting fresh re-pull for hierarchy.
4. Save buffer.
